...
def alhpa(src_graph, tar_graph, rsc=1, gt=None):
    """
    ...
    def cluster_recurse(src_e, tar_e, pos=[(0,0)]):
        ...
        try:
            print('computing Louvain (src graph)')
            partition = community_louvain.best_partition(G_s)
            _src_cluster = list(partition.values())
            src_nodes = list(partition.keys())
            K_s = len(set(_src_cluster))  # the number of clusters in the src graph
            ...
            return
        try:
            print('computing Louvain (tar graph)')
            partition = community_louvain.best_partition(G_t)
            _tar_cluster = list(partition.values())
            tar_nodes = list(partition.keys())
            K_t = len(set(_tar_cluster))  # the number of clusters in the tar graph
        except Exception as e:
            traceback.print_exc()
            print(repr(e))
            print(f'Troublesome graph!!!\n Matching parent graph using GRAMPA')
        ...
        K = max(K_s, K_t) # the maximum number of clusters in the two graphs
...
def main(data, rsc=1):
    ...
    n = Src.shape[0]
    s = time.time()
    matching, pos, readj_accs, cacc_pre, cacc_post = alhpa(Src, Tar, rsc, gt)
