...
def node_to_vec_impl(src_graph, tar_graph, rsc=0, weighting_scheme='ncut', lap=False, gt=None, ki=False, lalpha=10000):
    """
    ...
    def cluster_recurse(src_e, tar_e, pos=[(0,0)]):
        ...
        l, _ = spectral_embedding(src_adj, n_components=10)
        print('embedding tar graph...')
        mu, _ = spectral_embedding(tar_adj, n_components=10)
        ...
        src2vec = Node2Vec(G_s, dimensions=K, workers=8)
        tar2vec = Node2Vec(G_t, dimensions=K, workers=8)
        src_mod = src2vec.fit()
        tar_mod = tar2vec.fit()
        src_embedding = np.array([list(src_mod.wv.get_vector(n)) for n in G_s.nodes])
        tar_embedding = np.array([list(tar_mod.wv.get_vector(n)) for n in G_t.nodes])
