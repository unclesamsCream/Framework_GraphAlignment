...
def alhpa_mt(src_graph, tar_graph, rsc=0, weighting_scheme='ncut', lap=False, gt=None, ki=False, lalpha=10000, n_comp=10, n_threads=8):
    """
    ...
    def cluster_recurse(src_e, tar_e, pos=[(0,0)]):
        ...
        for i, (c_s, c_t) in enumerate(partition_alignment):
            ...
            if len(c_s_nodes) <= rsc or len(c_t_nodes) <= rsc:
                ...
                subproblem_queue.append((sub_src, sub_tar, pos))

        # cluster_recurse(src_graph, tar_graph)
    with subproblem_lock:
        subproblem_queue.append((src_graph, tar_graph))
    ...
    def thread_worker():
        with subproblem_lock:
            if subproblem_queue:
                subproblem_args = subproblem_queue.pop(0)
                cluster_recurse(*subproblem_args)
    ...
    while True:
        with subproblem_lock:
            # print(len(subproblem_queue))
            with threads_lock:
                # print(f'threads before: {threads}')
                threads = [x for x in threads if x.is_alive()]
                # print(f'threads after: {threads}')                
                if not subproblem_queue:
                    if len(threads) == 0:
                        break
        while len(threads) < n_threads and subproblem_queue:
            create_thread()
...
def alhpa_qr(src_graph, tar_graph, rsc=0, weighting_scheme='ncut', lap=False, gt=None, ki=False, lalpha=10000, n_comp=10, n_threads=8):
    """
    MAtching by Recursive Partition Alignment
    ____
    Summary or Description of the Function
...
def main(data, eta, rsc, lap, ki, lalpha, weighting_scheme, n_comp, n_threads, variation='QR'):
    print(f'\n\n\nstarting run:\nargs: (, eta, rsc, lap, ki, lalpha, weighting_scheme, n_comp, n_threads)={(eta, rsc, lap, ki, lalpha, weighting_scheme, n_comp, n_threads)}')
    Src = data['src_e']
    Tar = data['tar_e']
    gt = data["gt"]

    n = Src.shape[0]
    run_args = (Src, Tar, rsc, weighting_scheme, lap, gt, ki, lalpha, n_comp, n_threads)
    if variation == 'QR':
        matching, pos, readj_accs = alhpa_qr(*run_args)
    else:
        matching, pos, readj_accs = alhpa_mt(*run_args)
